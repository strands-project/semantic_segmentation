#ifndef _INFERENCE_GRAPH_HH_
#define _INFERENCE_GRAPH_HH_
...
#define INFINITE_D ((int)(((unsigned)-1)/2))
  ...
  public:
    ...
    // that can be added to the graph, and the second argument is an estimate
    // of the maximum number of edges. The last (optional) argument is the
    // pointer to the function which will be called if an error occurs;
    // an error message is passed to this function. If this argument is omitted,
    ...
    // than needed. Similarly for edges.
    ...
    // If num>1, then several nodes are added,
    // and node_id of the first one is returned.
    // IMPORTANT: see note about the constructor
    node_id add_node(int num = 1);

    // Adds a bidirectional edge between 'i' and 'j'
    ...
    // segment the node 'i' belongs
    // (Graph<captype,tcaptype,flowtype>::SOURCE or
  ...
  public:
    ...
    //   - To check that you marked all necessary nodes,
    //     you can call maxflow(false) after calling maxflow(true).
    //     If everything is correct, the two calls must return
    //     the same value of flow. (Useful for debugging).
    ...
    //    Then during the next call to maxflow(true, &changed_list) new nodes
    //    will be added to changed_list.
    //  - If the next call to maxflow() does not use option reuse_trees,
    //    then calling remove_from_changed_list() is not necessary.
    //   ("changed_list->Reset()" or "delete changed_list"
  ...
  private:
    ...
    struct node {
      ...
      int is_sink : 1; // flag showing whether the node is in the
      // source or in the sink tree (if parent!=NULL)
      int is_marked : 1; // set by mark_node()
      int is_in_changed_list : 1; // set by maxflow if
      tcaptype tr_cap; // if tr_cap > 0 then tr_cap is residual
      // capacity of the arc SOURCE->node
  ...
  template <typename captype, typename tcaptype, typename flowtype>
  inline void Graph<captype,tcaptype,flowtype>::add_tweights(node_id i,
                                                             tcaptype cap_source,
                                                             tcaptype cap_sink) {
  ...
  template <typename captype, typename tcaptype, typename flowtype>
  inline void Graph<captype,tcaptype,flowtype>::add_edge(node_id _i, node_id _j,
                                                         captype cap,
                                                         captype rev_cap) {
  ...
  template <typename captype, typename tcaptype, typename flowtype>
  inline void Graph<captype,tcaptype,flowtype>::get_arc_ends(arc* a,
                                                             node_id& i,
                                                             node_id& j) {
  ...
  template <typename captype, typename tcaptype, typename flowtype>
  inline void Graph<captype,tcaptype,flowtype>::set_trcap(node_id i,
                                                          tcaptype trcap) {
    assert(i>=0 && i<node_num);
  ...
  Graph<captype,tcaptype,flowtype>::what_segment(node_id i,
...
*/
  template <typename captype, typename tcaptype, typename flowtype>
  inline typename Graph<captype,tcaptype,flowtype>::node*
  ...
  template <typename captype, typename tcaptype, typename flowtype>
  flowtype Graph<captype,tcaptype,flowtype>::maxflow(bool reuse_trees,
    ...
    if (maxflow_iteration == 0 && reuse_trees) {
      if (error_function) {
        (*error_function)((char *)"reuse_trees cannot be used in the first call to maxflow()!");
      }
      exit(1);
    }
    ...
    if (changed_list && !reuse_trees) {
      if (error_function) {
        (*error_function)((char *)"changed_list cannot be used without reuse_trees!");
      }
      exit(1);
    }
  ...
  void Graph<captype,tcaptype,flowtype>::test_consistency(node* current_node) {
    ...
    // test whether all nodes i with i->next!=NULL are indeed in the queues
    for (i=nodes; i<node_last; i++) {
    ...
    for (i=nodes; i<node_last; i++) {
      ...
      // test whether passive nodes in search trees have neighbors
