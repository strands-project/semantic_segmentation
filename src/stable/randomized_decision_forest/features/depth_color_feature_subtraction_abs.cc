#include "depth_color_feature_subtraction_abs.hh"

using namespace Rdfs;

DepthColorFeatureSubtractionAbs::DepthColorFeatureSubtractionAbs(int x1, int y1, int c1, int x2, int y2, int c2):
  m_x1(x1), m_y1(y1), m_c1(c1), m_x2(x2), m_y2(y2), m_c2(c2){
  m_feature_type = HYBRID_SUB_ABS;
}


float DepthColorFeatureSubtractionAbs::ExtractFeature(const int x, const int y) const{
  ...
  x2 = std::max(x2, 0);
  y2 = std::max(y2, 0);
  return fabs(m_image_lab_data[3*(x1+m_image_width*y1)+m_c1] - m_image_lab_data[3*(x2+m_image_width*y2)+m_c2]);
}

void DepthColorFeatureSubtractionAbs::InitEvaluation(const Utils::DataImage &data_image) {
...
BOOST_CLASS_EXPORT_IMPLEMENT(DepthColorFeatureSubtractionAbs)
