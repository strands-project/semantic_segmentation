#include "depth_color_feature_addition.hh"

using namespace Rdfs;

DepthColorFeatureAddition::DepthColorFeatureAddition(int x1, int y1, int c1, int x2, int y2, int c2):
  m_x1(x1), m_y1(y1), m_c1(c1), m_x2(x2), m_y2(y2), m_c2(c2){
  m_feature_type = HYBRID_ADD;
}

float DepthColorFeatureAddition::ExtractFeature(const int x, const int y) const{
...
void DepthColorFeatureAddition::InitEvaluation(const Utils::DataImage &data_image) {
...
BOOST_CLASS_EXPORT_IMPLEMENT(DepthColorFeatureAddition)
