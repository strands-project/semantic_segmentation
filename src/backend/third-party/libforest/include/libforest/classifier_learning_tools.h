...
namespace libf {
    ...
    public:
        ...
        static void updateHistograms(std::shared_ptr<T> tree, AbstractDataStorage::ptr storage, float smoothingParameter)
        {
          ...
          for(int sample = 0; sample < storage->getSize(); ++sample){
            dist[storage->getClassLabel(sample)]++;
          }
          ...
          for(int cls = 0; cls < C; ++cls){
            ...
            for (int v = 0; v < tree->getNumNodes(); v++)
            {
                if (tree->getNodeConfig(v).isLeafNode())
                {
                    ...
                    for (int c = 0; c < C; c++)
                    {
                      hist[c] = /*std::log(*/(hist[c]*dist[c] + smoothingParameter)/(total + C*smoothingParameter)/*)*/;
                    }
                }
            }
        }
