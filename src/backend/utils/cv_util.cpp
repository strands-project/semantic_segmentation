...
void ShowCvMat(const cv::Mat& m, std::string window_name, bool blocking) {
...
void ShowCvMatHeatMap(const cv::Mat& m, std::string window_name, bool blocking, int cm) {
    double min, max;
    ...
    ShowCvMat(temp, window_name, blocking);
}
